function goGPS_LS_SA_variometric_a(time_rx_t0, time_rx_t1, time_rx_t2, pr1_t0, pr1_t1, pr1_t2, pr2_t0, pr2_t1, pr2_t2, ph1_t0, ph1_t1, ph1_t2, ph2_t0, ph2_t1, ph2_t2, snr_t0, snr_t1, snr_t2, Eph_t0, Eph_t1, Eph_t2, SP3_t0, SP3_t1, SP3_t2, iono, sbas_t0, sbas_t1, sbas_t2, lambda, phase, time_step, do)
...
% NEW VERSION
     if (phase == 1)
        ...
        sat_pr_t2 = find( (pr1_t2 ~= 0)  );
        sat_t2 = find( (pr1_t2 ~= 0) & (ph1_t2 ~= 0) );
    else
        sat_pr_t0 = find( (pr2_t0 ~= 0) );
        sat_t0 = find( (pr2_t0 ~= 0) & (ph2_t0 ~= 0) );
        sat_pr_t1 = find( (pr2_t1 ~= 0) );
        sat_t1 = find( (pr2_t1 ~= 0) & (ph2_t1 ~= 0) );
        sat_pr_t2 = find( (pr2_t2 ~= 0) );
        sat_t2 = find( (pr2_t1 ~= 0) & (ph2_t2 ~= 0) );
    end

%end
sat_pr = intersect(sat_pr_t0,sat_pr_t1);
sat_pr = intersect(sat_pr,sat_pr_t2);
sat = intersect(sat_t0,sat_t1);
sat = intersect(sat,sat_t2);
...
if (size(sat,1) >= min_nsat_LS)
    ...
    if (phase == 1)
        [XR_t0, dtR_t0, XS_t0, dtS_t0, XS_tx_t0, VS_tx_t0, time_tx_t0, err_tropo_t0, err_iono_t0, sat_pr, elR(sat_pr), azR(sat_pr), distR(sat_pr), sys, cov_XR_t0, var_dtR_t0, PDOP, HDOP, VDOP, cond_num] = init_positioning(time_rx_t0, pr1_t0(sat_pr), snr_t0(sat_pr), Eph_t0, SP3, iono, sbas_t0, [], [], [], sat_pr, [], lambda(sat_pr,:), cutoff, snr_threshold, phase, 0, 0); %#ok<ASGLU>
        ...
        [XR_t2, dtR_t2, XS_t2, dtS_t2, XS_tx_t2, VS_tx_t2, time_tx_t2, err_tropo_t2, err_iono_t2, sat_pr, elR(sat_pr), azR(sat_pr), distR(sat_pr), sys, cov_XR_t2, var_dtR_t2, PDOP, HDOP, VDOP, cond_num] = init_positioning(time_rx_t2, pr1_t2(sat_pr), snr_t2(sat_pr), Eph_t2, SP3, iono, sbas_t2, [], [], [], sat_pr, [], lambda(sat_pr,:), cutoff, snr_threshold, phase, 0, 0); %#ok<ASGLU>
        elR_t2(sat_pr) = elR(sat_pr);
        azR_t2(sat_pr) = azR(sat_pr);
        distR_t2(sat_pr) = distR(sat_pr);
        sat_pr_t2 = sat_pr;
    else
    ...
    if (length(sat_pr_t0) ~= length(sat_pr_t1)) || (length(sat_pr_t0) ~= length(sat_pr_t2))
        % sat_pr_t0
        % sat_pr_t1
        sat_pr = intersect(sat_pr_t0,sat_pr_t1);
        sat_pr = intersect(sat_pr,sat_pr_t2);
        if size(sat_pr,1) <= min_nsat_LS
            Xhat_t_t = [0; 9999;  0; 9999; 0; 9999; 0; 9999;  0; 9999; 0; 9999; 0; 9999;  0; 9999; 0; 9999];
            return
        end
        [XR_t0, dtR_t0, XS_t0, dtS_t0, XS_tx_t0, VS_tx_t0, time_tx_t0, err_tropo_t0, err_iono_t0, sat_pr, elR(sat_pr), azR(sat_pr), distR(sat_pr), sys, cov_XR_t0, var_dtR_t0, PDOP, HDOP, VDOP, cond_num] = init_positioning(time_rx_t0, pr1_t0(sat_pr), snr_t0(sat_pr), Eph_t0, SP3, iono, sbas_t0, [], [], [], sat_pr, [], lambda(sat_pr,:), cutoff, snr_threshold, phase, 0, 0); %#ok<ASGLU>
        elR_t0(sat_pr) = elR(sat_pr);
...
if isempty(cov_XR_t0) %if it was not possible to compute the covariance matrix
    cov_XR_t0 = sigmaq0 * eye(3);
end
...
if isempty(cov_x_hat) %if it was not possible to compute the covariance matrix
    cov_x_hat = sigmaq0 * eye(3);
    cov_x_hat_a = sigmaq0 * eye(3);
end
sigma2_x_hat = diag(cov_x_hat);
sigma2_x_hat_a = diag(cov_x_hat_a);
...
Xhat_t_t = [XR_t0(1);cov_XR_t0(1);XR_t0(2); cov_XR_t0(5); XR_t0(3);cov_XR_t0(9); x_hat(1);sigma2_x_hat(1);  x_hat(2);sigma2_x_hat(2);  x_hat(3);sigma2_x_hat(3);x_hat_a(1);sigma2_x_hat_a(1);  x_hat_a(2);sigma2_x_hat_a(2);  x_hat_a(3);sigma2_x_hat_a(3)];
