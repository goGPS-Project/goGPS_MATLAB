function goGPS_master_monitor(filerootOUT)
...
global ntrip_ip ntrip_port nmea_init
global master
...
while flag
    ...
    if (master_1 == master_2) & (master_1 ~= 0)
        ...
        [cell_master] = decode_rtcm(data_master);
        ...
        for i = 1 : size(cell_master,2)
            ...
            elseif (cell_master{1,i} == 1003)
                ...
                ph2_M(find(ph2_M < 1e-100)) = 0;

                %counter increment
                t = t+1;

                %data save
                fwrite(fid_obs, [0; time_M; 0; pr1_M; zeros(32,1); ph1_M; zeros(32,1); snr1_M; zeros(32,1)], 'double');
                fwrite(fid_eph, [0; Eph(:)], 'double');
            ...
            elseif (cell_master{1,i} == 1010)
                ...
                %manage "nearly null" data
                ph_M_GLO(find(ph_M_GLO < 1e-100)) = 0;
            ...
            elseif (cell_master{1,i} == 1011)
                ...
                %manage "nearly null" data
                ph1_M_GLO(find(ph1_M_GLO < 1e-100)) = 0;
                ph2_M_GLO(find(ph2_M_GLO < 1e-100)) = 0;
