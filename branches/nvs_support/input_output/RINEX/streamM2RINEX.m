function streamM2RINEX(fileroot, filename, week, rinex_metadata, constellations, wait_dlg)

% SYNTAX:
%   streamM2RINEX(fileroot, filename, week, rinex_metadata, constellations, wait_dlg);
%
% INPUT:
...
if (~isempty(data_master_all))
    ...
    if (~isempty(cell_master))
        ...
        fid_obs = fopen([filename sprintf('.%2do', two_digit_year(date(1,1)))],'wt');
        ...
        if (~isempty(find(Eph_M(1,:,:) ~= 0, 1)))
            ...
            for f = 1 : length(ext)
                ...
                fid_nav = fopen([filename sprintf('.%2d%c', two_digit_year(date(1,1)), ext(f))], 'wt');
                ...
                for i = 1 : N
                    ...
                    for j = 1 : length(idxEph)
                        ...
                        if ((strcmp(ext(f),'n') &&  strcmp(char(Eph_M(31,idxEph(j),i)),'G')) || ...
                            ...
                            date_toc = gps2date(week_M(i), toc);
                        ...
                        elseif ((strcmp(ext(f),'g') || strcmp(ext(f),'p')) && strcmp(char(Eph_M(31,idxEph(j),i)),'R'))
                            ...
                            %date_GPS = gps2date(week_M(i), time_M(i));
                            date_GPS = date(i,:);
