...
if (size(sat,1) >= 4)
    ...
    if (phase == 1)
        ...
        if (length(sat_M) < 4); conf_sat=zeros(32,1); conf_sat(sat_M)=1; return; end
    ...
    if (size(sat,1) >= 4 & cond_num < cond_num_threshold)
        ...
        for i = 1 : 3
            if (phase == 1)
                [XR, N1_hat, cov_XR, cov_N1, PDOP, HDOP, VDOP, up_bound, lo_bound, posType] = LS_DD_code_phase(XR, XM, XS, pr1_R(sat), ph1_R(sat), snr_R(sat), pr1_M(sat), ph1_M(sat), snr_M(sat), elR(sat), elM(sat), err_tropo_R, err_iono_R, err_tropo_M, err_iono_M, pivot_index, phase);
            else
