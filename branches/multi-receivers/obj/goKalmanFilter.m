...
classdef goKalmanFilter < handle
    ...
    methods (Access = 'private')
        ...
        function init_Xhat_t_t(obj, goObs, goIni, mode)   %% to initialize Xhat_t_t_R: cell of [nPar+nSat*nFreq,1] and Xhat_t_t;
            ...
            pr1_R=zeros(MAX_SAT,length(time_M),nRec);
            ph1_R=zeros(MAX_SAT,length(time_M),nRec);
            snr_R=zeros(MAX_SAT,length(time_M),nRec);
            dop1_R=zeros(MAX_SAT,length(time_M),nRec);
            pr2_R=zeros(MAX_SAT,length(time_M),nRec);
            ph2_R=zeros(MAX_SAT,length(time_M),nRec);
            dop2_R=zeros(MAX_SAT,length(time_M),nRec);
            ...
            XR_DD=NaN(3,length(time_GPS),nRec);
            progbar = progressBar(length(time_GPS),'Code+Phase DD MASTER - epoch');
