...
if (nsat >= min_nsat)
    ...
    if (phase == 1)
        ...
        [ ~, dtR, XS, dtS,     ~,     ~,       ~, err_tropo_R, err_iono_R, sat_pr_R, elR(sat_pr_R), azR(sat_pr_R), distR(sat_pr_R), cov_XR, var_dtR] = init_positioning(time_rx, pr1_R(sat_pr_M), snr_R(sat_pr_M), Eph, SP3_time, SP3_coor, SP3_clck, iono, [], XR0, XS, dtS, sat_pr_M, cutoff, snr_threshold, flag_XR, 1); %#ok<NASGU,ASGLU>
    else
        ...
        [ ~, dtR, XS, dtS,     ~,     ~,       ~, err_tropo_R, err_iono_R, sat_pr_R, elR(sat_pr_R), azR(sat_pr_R), distR(sat_pr_R), cov_XR, var_dtR] = init_positioning(time_rx, pr2_R(sat_pr_M), snr_R(sat_pr_M), Eph, SP3_time, SP3_coor, SP3_clck, iono, [], XR0, XS, dtS, sat_pr_M, cutoff, snr_threshold, flag_XR, 1); %#ok<NASGU,ASGLU>
    end
    ...
    if (nsat >= min_nsat)
        ...
        if ~isempty(sat)
            ...
            if (length(phase) == 2)
                [check_cs1, N_slip1, sat_slip1] = cycle_slip_detection(X_t1_t(o1+1:o1+32), ph1_R(sat), ph1_M(sat), distR(sat), distM(sat), doppler_pred_range1_R(sat), doppler_pred_range1_M(sat), pivot, sat, sat_born, cs_threshold, 1); %#ok<ASGLU>
                [check_cs2, N_slip2, sat_slip2] = cycle_slip_detection(X_t1_t(o1+1:o1+32), ph2_R(sat), ph2_M(sat), distR(sat), distM(sat), doppler_pred_range2_R(sat), doppler_pred_range2_M(sat), pivot, sat, sat_born, cs_threshold, 2); %#ok<ASGLU>
                ...
                if (phase == 1)
                    [check_cs, N_slip, sat_slip] = cycle_slip_detection(X_t1_t(o1+1:o1+32), ph1_R(sat), ph1_M(sat), distR(sat), distM(sat), doppler_pred_range1_R(sat), doppler_pred_range1_M(sat), pivot, sat, sat_born, cs_threshold, 1); %#ok<ASGLU>
                else
                    [check_cs, N_slip, sat_slip] = cycle_slip_detection(X_t1_t(o1+1:o1+32), ph2_R(sat), ph2_M(sat), distR(sat), distM(sat), doppler_pred_range2_R(sat), doppler_pred_range2_M(sat), pivot, sat, sat_born, cs_threshold, 2); %#ok<ASGLU>
                end
        ...
        if ~isempty(p)
            ...
            if (length(phase) == 2)
                H_pha1(:,o1+1 :o1+32) = L_pha1(p,:);
                H_pha2(:,o1+33:o1+64) = L_pha2(p,:);
                H_pha = [H_pha1; H_pha2];
            else
                if (phase == 1)
                    H_pha1(:,o1+1:o1+32) = L_pha1(p,:);
