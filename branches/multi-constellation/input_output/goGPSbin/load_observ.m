...
if ~isempty(time_GPS)
    ...
    if ~isempty(time_R)
        ...
        for i = 1 : length(newtime_R)
            ...
            Eph_R  = cat(3, Eph_R(:,:,1:pos), zeros(29,num_sat,1), Eph_R(:,:,pos+1:end));
        ...
        Eph_R  = zeros(29,num_sat,length(time_GPS));
    ...
    if ~isempty(time_M)
        ...
        for i = 1 : length(newtime_M)
            ...
            Eph_M  = cat(3, Eph_M(:,:,1:pos), zeros(29,num_sat,1), Eph_M(:,:,pos+1:end));
        ...
        Eph_M  = zeros(29,num_sat,length(time_GPS));
    end
