...
%       snr_MR(delsat,:) = 0;
   ...
   elseif (mode_data == 1)
      ...
      [time_GPS, time_R, time_M, pr1_R, pr1_M, ph1_R, ph1_M, snr_R, snr_M, ...
       Eph, delay, loss_R, loss_M] = load_goGPSinput (filerootIN);

      %reference GPS time
      time_GPS = time_GPS(1) + (0 : 1 : length(time_M)-1)';

      %nominal date for Google Earth
      data0 = datenum(2009,1,1,0,0,0);
      data = datevec(data0 + time_GPS/86400);
...
%MASTER position (if it is known "a priori")
pos_M = [XM; YM; ZM];
...
if (mode == 1) & (mode_vinc == 0)
   ...
   kalman_goGPS_init (pos_M, time_GPS(1), Eph_t, iono_R, pr1_R(:,1), pr1_M(:,1), ph1_R(:,1), ph1_M(:,1), pr2_R(:,1), pr2_M(:,1), ph2_R(:,1), ph2_M(:,1), [1]);
   ...
   if (flag_cov == 0)
       if (flag_ge == 1), rtplot_googleearth (1, [Xhat_t_t(1); Xhat_t_t(o1+1); Xhat_t_t(o2+1)], link_filename, kml_filename, data(1,:)), end;
       rtplot_matlab (1, [Xhat_t_t(1); Xhat_t_t(o1+1); Xhat_t_t(o2+1)], 0, 0, 0, 0, flag_ms, ref_path, mat_path);
   else
       if (flag_ge == 1), rtplot_googleearth_cov (1, [Xhat_t_t(1); Xhat_t_t(o1+1); Xhat_t_t(o2+1)], Cee([1 o1+1 o2+1],[1 o1+1 o2+1]), link_filename, kml_filename, data(1,:)), end;
       rtplot_matlab_cov (1, [Xhat_t_t(1); Xhat_t_t(o1+1); Xhat_t_t(o2+1)], Cee([1 o1+1 o2+1],[1 o1+1 o2+1]), 0, 0, 0, 0, flag_ms, ref_path, mat_path);
   end
   %rtplot_amb (1, window, Xhat_t_t(o3+1:o3+32), sqrt(diag(Cee(o3+1:o3+32,o3+1:o3+32))), conf_cs, 2);
   rtplot_skyplot (azR, elR, conf_sat, pivot);
   rtplot_snr (snr_R(:,1), 1);
   ...
   for t = 2 : length(time_GPS)
      ...
      [check_on, check_off, check_pivot, check_cs] = kalman_goGPS_loop (pos_M, time_GPS(t), Eph_t, iono_R, pr1_R(:,t), pr1_M(:,t), ph1_R(:,t), ph1_M(:,t), pr2_R(:,t), pr2_M(:,t), ph2_R(:,t), ph2_M(:,t), snr_R(:,t), snr_M(:,t), [1]);
      ...
      if (flag_cov == 0)
          if (flag_ge == 1), rtplot_googleearth (t, [Xhat_t_t(1); Xhat_t_t(o1+1); Xhat_t_t(o2+1)], link_filename, kml_filename, data(t,:)), end;
          rtplot_matlab (t, [Xhat_t_t(1); Xhat_t_t(o1+1); Xhat_t_t(o2+1)], check_on, check_off, check_pivot, check_cs, flag_ms, ref_path, mat_path);
      else
          if (flag_ge == 1), rtplot_googleearth_cov (t, [Xhat_t_t(1); Xhat_t_t(o1+1); Xhat_t_t(o2+1)], Cee([1 o1+1 o2+1],[1 o1+1 o2+1]), link_filename, kml_filename, data(t,:)), end;
          rtplot_matlab_cov (t, [Xhat_t_t(1); Xhat_t_t(o1+1); Xhat_t_t(o2+1)], Cee([1 o1+1 o2+1],[1 o1+1 o2+1]), check_on, check_off, check_pivot, check_cs, flag_ms, ref_path, mat_path);
      end
      %rtplot_amb (t, window, Xhat_t_t(o3+1:o3+32), sqrt(diag(Cee(o3+1:o3+32,o3+1:o3+32))), conf_cs, 2);
      rtplot_skyplot (azR, elR, conf_sat, pivot);
      rtplot_snr (snr_R(:,t), 1);
...
elseif (mode == 1) & (mode_vinc == 1)
   ...
   kalman_goGPS_vinc_init (pos_M, time_GPS(1), Eph_t, iono_R, pr1_R(:,1), pr1_M(:,1), ph1_R(:,1), ph1_M(:,1), pr2_R(:,1), pr2_M(:,1), ph2_R(:,1), ph2_M(:,1), [1], ref_loop);

   fwrite(fid_kal, [Xhat_t_t; Yhat_t_t; Cee(:)], 'double');
   fwrite(fid_sat, [azM; azR; elM; elR; distM; distR], 'double');
   fwrite(fid_conf, [conf_sat; conf_cs; pivot], 'int8');

   if (flag_ge == 1), rtplot_googleearth (1, [Yhat_t_t(1); Yhat_t_t(2); Yhat_t_t(3)], link_filename, kml_filename, data(1,:)), end;
   rtplot_matlab (1, [Yhat_t_t(1); Yhat_t_t(2); Yhat_t_t(3)], 0, 0, 0, 0, flag_ms, ref_path, mat_path);
   %rtplot_amb (1, window, Xhat_t_t(o1+1:o1+32), sqrt(diag(Cee(o1+1:o1+32,o1+1:o1+32))), conf_cs, 2);
   rtplot_skyplot (azR, elR, conf_sat, pivot);
   rtplot_snr (snr_R(:,1), 1);
   ...
   for t = 2 : length(time_GPS)
      ...
      [check_on, check_off, check_pivot, check_cs] = kalman_goGPS_vinc_loop (pos_M, time_GPS(t), Eph_t, iono_R, pr1_R(:,t), pr1_M(:,t), ph1_R(:,t), ph1_M(:,t), pr2_R(:,t), pr2_M(:,t), ph2_R(:,t), ph2_M(:,t), snr_R(:,t), snr_M(:,t), [1], ref_loop);

      fwrite(fid_kal, [Xhat_t_t; Yhat_t_t; Cee(:)], 'double');
      fwrite(fid_sat, [azM; azR; elM; elR; distM; distR], 'double');
      fwrite(fid_conf, [conf_sat; conf_cs; pivot], 'int8');

      if (flag_ge == 1), rtplot_googleearth (t, [Yhat_t_t(1); Yhat_t_t(2); Yhat_t_t(3)], link_filename, kml_filename, data(t,:)), end;
      rtplot_matlab (t, [Yhat_t_t(1); Yhat_t_t(2); Yhat_t_t(3)], check_on, check_off, check_pivot, check_cs, flag_ms, ref_path, mat_path);
      %rtplot_amb (t, window, Xhat_t_t(o1+1:o1+32), sqrt(diag(Cee(o1+1:o1+32,o1+1:o1+32))), conf_cs, 2);
      rtplot_skyplot (azR, elR, conf_sat, pivot);
      rtplot_snr (snr_R(:,t), 1);
...
elseif (mode == 3)
   ...
   MQ_goGPS_loop (time_GPS(1), Eph_t, pos_M, pr1_R(:,1), pr1_M(:,1), pr2_R(:,1), pr2_M(:,1), [1]);
   ...
   for t = 2 : length(time_GPS)
      ...
      MQ_goGPS_loop (time_GPS(t), Eph_t, pos_M, pr1_R(:,t), pr1_M(:,t), pr2_R(:,t), pr2_M(:,t), [1]);
...
elseif (mode == 5)
   ...
   kalman_goGPS_cod_init (pos_M, time_GPS(1), Eph_t, iono_R, pr1_R(:,1), pr1_M(:,1), pr2_R(:,1), pr2_M(:,1), [1]);
   ...
   for t = 2 : length(time_GPS)
      ...
      [check_on, check_off, check_pivot, check_cs] = kalman_goGPS_cod_loop (pos_M, time_GPS(t), Eph_t, iono_R, pr1_R(:,t), pr1_M(:,t), pr2_R(:,t), pr2_M(:,t), snr_R(:,t), snr_M(:,t), [1]);
...
%[time_GPS, time_R, time_M, pr1_R, pr1_M, ph1_R, ph1_M, snr_R, snr_M, ...
% Eph, delay, loss_R, loss_M] = load_goGPSinput (filerootOUT);
...
%
%    coltab = jet;
%    coltab = [1 1 1; coltab([1 8 16 24 32 40 48 56 64],:)];
%
%    figure
%    subplot(2,1,1);
%    imagesc(snr_R.*abs(conf_sat),[-0.5 9.5]);
%    title('Rover S/N ratio');
%    axis xy; colormap(coltab);
%    h = colorbar; set(h,'YTick',0:9);
%    subplot(2,1,2);
%    imagesc(snr_M.*abs(conf_sat),[-0.5 9.5]);
...
if (mode < 10) & (mode_vinc == 0) & (~isempty(ref_path))
    ...
    [dist2D, proj] = ref_2d_projection(ref,EST_KAL,NORD_KAL);
