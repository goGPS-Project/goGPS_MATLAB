function [zi,s2zi] = krigingSph(vstruct,x,y,z,xi,yi,chunksize)

% interpolation with ordinary kriging in two dimensions
%
% Syntax:
%
%     [zi,zivar] = krigingSph(vstruct,x,y,z,xi,yi)
%     [zi,zivar] = krigingSph(vstruct,x,y,z,xi,yi,chunksize)
%
% Description:
%
...
%     x,y       coordinates of observations in [deg]
...
[X1,X2]=meshgrid(x,x);
[Y1,Y2]=meshgrid(y,y);
Dx = 2 * asind(sqrt(sind((Y1 - Y2) / 2) .^2 + cosd(Y1) .* cosd(Y2) .* sind((X1 - X2) / 2).^2));
% Dany: Dx = abs(acosd(cosd(Y1) .* cosd(Y2) .* cosd(X1 - X2) + sind(Y1) .* sind(Y2)));
clear X1 X2 Y1 Y2;
...
for r = 1:nrloops
    ...
    [X1,X2] = meshgrid(xi(IX),x);
    [Y1,Y2] = meshgrid(yi(IX),y);
    b = 2 * asind(sqrt(sind((Y1 - Y2) / 2) .^2 + cosd(Y1) .* cosd(Y2) .* sind((X1 - X2) / 2).^2));
    % Dany: b = abs(acosd(cosd(Y1) .* cosd(Y2) .* cosd(X1 - X2) + sind(Y1) .* sind(Y2)));
    clear X1 X2 Y1 Y2;
