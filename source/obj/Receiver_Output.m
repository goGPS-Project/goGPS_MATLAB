    ...
    methods
        function injectResult(this, rec_work, rate, flag_force)
            ...
            if flag_ok && ~isempty(rec_work.quality_info.s0) && ~isnan(rec_work.quality_info.s0) && rec_work.quality_info.s0 > 0.25
                log.addError(sprintf('Receiver "%s" processing sigma0 is above 5cm, this means that it probably failed, not importing results as output', this.parent.getMarkerName4Ch()));
                flag_ok = false;
            end
            % -------------------------------------------------------------
            ...
            if flag_force || (flag_ok && (~(rec_work.isEmpty || rec_work.flag_currupted)))
                ...
                if ~work_time.isEmpty
                    ...
                    if (state.flag_out_dt || state.isTropoOut || state.flag_out_ocs || state.isNSatOut || state.flag_out_quality || state.flag_out_azel)
                        ...
                        for i = 1 : length(rec_work.add_coo)
                            ...
                            if not(coo_to_import.isEmpty)
                                if i > numel(this.add_coo) || isempty(this.add_coo(i).coo)
                                    ...
                                    try
                                        time_o = coo_to_import.time.getCopy();
                                        coo_o = coo_to_import.getCopy();
