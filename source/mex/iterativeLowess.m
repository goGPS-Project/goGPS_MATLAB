...
%   frac_seconds: window size for local regression in seconds
%   iterations: number of iterations for weighting scheme
%   lambda: regularization parameter
%
% SYNTAX:
%   smoothed = iterativeLowess(time, data, frac_seconds, iterations, lambda)
...
function smoothed = iterativeLowess(time, data, frac_seconds, iterations, lambda)    

    if (nargin <= 2) || isempty(frac_seconds)
        % Estimate frac_seconds
        total_time_seconds = (max(time) - min(time)) * 24 * 60 * 60; % Convert days to seconds
        frac_seconds = 0.1 * total_time_seconds;
    end
    ...
    frac_days = frac_seconds / (24*60*60);  % Convert frac from seconds to days
