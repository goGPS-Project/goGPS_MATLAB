...
for s = 1 : num_session
    ...
    if state.isModeSEID()
        for r = 1 : num_ref_rec
            filename_R_obs{r} = ref_rec{r}{s}; %#ok<SAGROW>
        end
        filename_M_obs = trg_rec{1}{s};
    else
        for r = 1 : num_trg_rec
            filename_R_obs{r} = trg_rec{r}{s}; %#ok<SAGROW>
        end
        if state.isModeDD()
            for r = 1 : num_mst_rec
                filename_M_obs{r} = mst_rec{r}{s}; %#ok<SAGROW>
            end
        end
    end
    ...
    elseif (mode == goGNSS.MODE_PP_LS_C_DD)
        ...
        for t = 1 : length(time_GPS)
            ...
            goGPS_LS_DD_code(time_GPS_diff(t), pos_R, pos_M(:,t), pr1_R(:,t), pr1_M(:,t), pr2_R(:,t), pr2_M(:,t), snr_R(:,t), snr_M(:,t), Eph_t, SP3, iono, lambda, frequencies(1));
