function rtplot_matlab_stopGOstop (t, pos_R, pos_M, P1, P2, origin, ref, matrix, flag_dyn, flag_amb)

% SYNTAX:
%   rtplot_matlab_stopGOstop (t, pos_R, pos_M, P1, P2, origin, ref, matrix, flag_dyn, flag_amb);
%
% INPUT:
%   t = survey time (t=1,2,...)
...
elseif (flag_dyn == 1) && (pid(1) ~= 0)
    set(pid(1), 'XData', EAST-EAST_O, 'YData', NORTH-NORTH_O, 'Color', 'b');
elseif (flag_dyn == 2)
    plot(EAST-EAST_O, NORTH-NORTH_O, '.g');
elseif (flag_dyn == 3) && (pid(2) == 0)
    pid(2) = plot(EAST-EAST_O, NORTH-NORTH_O, '.b');
elseif (flag_dyn == 3) && (pid(2) ~= 0)
    set(pid(2), 'XData', EAST-EAST_O, 'YData', NORTH-NORTH_O, 'Color', 'b');
end
