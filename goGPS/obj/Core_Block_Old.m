...
classdef Core_Block_Old < handle
    ...
    properties (Access = public)% Public Access
        ...
        sol_type = Core_Block_Old.FLAG_PHASE_ONLY
    ...
    methods (Static)
        function this = Core_Block_Old(n_epoch, n_pr_obs, n_ph_obs)
                        ...
                        if this.state.isOutlierRejectionOn()
                            ...
                            if size(A,1) < 0.6 * n_obs_tmp
                                ...
                                if full_slip_split && (size(lim, 1) > 1) && (size(lim, 2) > 1) && (s02 > 0.02)
                                    ...
                                    %[x, Cxx, s02, v_hat_ck] = Core_Block_Old.solveLS(y0, b, A_ck, col_ok_ck, iQ);
...
%                 end
    ...
    methods (Static) % Public Access
        ...
        function ref = getAmbCorrection(data, res, win_size)
            %ref1 = Core_Block_Old.improveRef(data, movmedian(round(data), win_size, 'omitnan'), win_size);
            %ref2 = Core_Block_Old.improveRef(data, movmedian(round(data+0.5) - 0.5, win_size, 'omitnan'), win_size);
            ...
            % test ref on data
            ref = Core_Block_Old.improveRef(res, ref, win_size);
        ...
        function go_block = go (time_diff, pos_R, pos_M, pr1_R, pr1_M, pr2_R, pr2_M, ph1_R, ph1_M, ph2_R, ph2_M, snr_R, snr_M,  Eph, sp3, iono, lambda, ant_pcv, s_rate)
            ...
            % go_block = Core_Block_Old.go(time_GPS_diff, pos_R, pos_M, pr1_R, pr1_M, pr2_R, pr2_M, ph1_R, ph1_M, ph2_R, ph2_M, snr_R, snr_M,  Eph, SP3, iono, lambda, antenna_PCV, 3600);
            if nargin == 18
                s_rate = numel(time_diff);
            end
            go_block = Core_Block_Old (numel(time_diff), sum(serialize(pr1_R(:,:,1) ~= 0)), sum(serialize(ph1_R(:,:,1) ~= 0)));
        ...
        function go_block = getCopyFrom(obj)
            % get a copy of a Core_Block_Old instance
            % SYNTAX: go_block = Core_Block_Old.getCopyFrom(obj);
            go_block = Core_Block_Old(0,0,0);
            go_block.import(obj);
        end
    end
    ...
    methods (Static) % Public Access
        ...
        function [x, Cxx, s02, v_hat, P_out, N_out, Cyy] = solveLS(y0, b, A, col_ok, iQ, P, N)
            ...
            % SYNTAX: [x, Cxx, s02, v_hat, P_out, N_out, Cyy] = Core_Block_Old.solveLS(y0, b, A, col_ok, cholinv(Q), P, N)
            [n_obs, n_col] = size(A);
        ...
        function [col_ok, ref_arc] = getBestRefArc(y0, b, A, iQ)
            ...
            if (amb_num > 1)
                ...
                for i = 4 : size(A,2)
                    ...
                    [x, Cxx, s02, v_hat, P, N] = Core_Block_Old.solveLS(y0, b, A, col_ok, iQ, P, N);
                ...
                if ~isempty(bad_block)
                    ...
                    if (amb_num > 1)
                        ...
                        for i = 1 : numel(block_arc)
                            ...
                            [~, Cxx, ~, ~, P, N] = Core_Block_Old.solveLS(y0, b, A, col_ok, iQ, P, N);
                            amb_var = Cxx(4:end,4:end);
                            amb_var(amb_var < 0) = 1e30;
