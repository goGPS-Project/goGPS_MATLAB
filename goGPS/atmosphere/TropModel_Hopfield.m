...
%USAGE:
%     [tropTOTAL,tropDRY,tropWET]=TropModel_Hopfield(ReceiverPos,SatPos,...*
...
function [tropTOTAL,tropDRY,tropWET]=TropModel_Hopfield(ReceiverPos,SatPos,...
                                           Temperature,Pressure,RelHumidity)
...
try
    ...
    N_wet0 = k3 .* (e ./ T.^2);%Wet refractivity along signal path @ MSL
catch
    ...
    N_dry0 = k1 .*( P ./ T);  %Dry refractivity along signal path @ MSL
    N_wet0 = k2.*(e./ T)+k3.*( e ./ T.^2);%Wet refractivity along signal path @ MSL
end %try

%COMPUTE MEAN DRY TROPOSPHERE HEIGHT(Hd)
Hd = 40136 + 148.72 .* (T - 273.16);% meters

% DEFINE MEAN DRY TROPOSPHERE HEIGHT(Hw)
Hw = 11000;% meters

%COMPUTE TOTAL TROPOSPHERIC REFRACTION EFFECT IN THE... 
%                             DIRECTION TO THE ZENITH
Kd=(1.0e-6 / 5) .* N_dry0 .* Hd;
Kw=(1.0e-6 / 5) .* N_wet0 .* Hw;
...
try
   MFh(j,1)=1./sin(sqrt(satEL.^2+1.904*10^-3));%Hydrostatic Mapping Function
   MFw(j,1)=1./sin(sqrt(satEL.^2+6854*10^-3));%Wet Mapping Function
catch
     MFh(j,1)=1./sin(sqrt(satEL.^2+6.25));%Hydrostatic Mapping Function
     MFw(j,1)=1./sin(sqrt(satEL.^2+2.25));%Wet Mapping Function
end
% %COMPUTE %Zenith Hydrostatic Delay(ZHD)
 zhd(j,1)=Kd(i,1).*MFh(j,1);

%COMPUTE Zenith Wet Delay(ZWD)
zwd(j,1)=Kw(i,1).*MFw(j,1);
