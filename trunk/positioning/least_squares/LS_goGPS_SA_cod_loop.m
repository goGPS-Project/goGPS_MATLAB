function LS_goGPS_SA_cod_loop(time, Eph_R, pr1_R, pr2_R, snr_R, iono, phase)

% SYNTAX:
%   LS_goGPS_SA_cod_loop(time, Eph_R, pr1_R, pr1_M, pr2_R, snr_R, iono, phase);
%
% INPUT:
%   time = GPS time
...
if (size(sat_pr,1) >= 4)
    ...
    if (size(sat_pr,1) >= 4)
        if (phase == 1)
            [pos_LS_SA, cov_pos_LS_SA, PDOP, HDOP, VDOP] = code_SA(pos_R, pr1_R(sat_pr), snr_R(sat_pr), sat_pr, time, Eph_R, iono);
        else
            [pos_LS_SA, cov_pos_LS_SA, PDOP, HDOP, VDOP] = code_SA(pos_R, pr2_R(sat_pr), snr_R(sat_pr), sat_pr, time, Eph_R, iono);
        end
    else
        if (~isempty(Xhat_t_t))
