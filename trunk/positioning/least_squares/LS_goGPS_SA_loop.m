...
if (length(phase) == 2)
    sat_pr = find( (pr1_R ~= 0) & (pr2_R ~= 0) ); %#ok<NASGU>
    sat = find( (pr1_R ~= 0) & (ph1_R ~= 0) & ...
        (pr2_R ~= 0) & (ph2_R ~= 0) );
else
    if (phase == 1)
        sat_pr = find( (pr1_R ~= 0) ); %#ok<NASGU>
        sat = find( (pr1_R ~= 0) & (ph1_R ~= 0) );
    else
        sat_pr = find( (pr2_R ~= 0) ); %#ok<NASGU>
...
if (size(sat_pr,1) >= 4)
    ...
    if (size(sat_pr,1) >= 4)
        if (phase == 1)
            [pos_LS_SA, cov_pos_LS_SA, N_stim, cov_N_stim, PDOP, HDOP, VDOP] = code_phase_SA(pos_R(1:3), pr1_R(sat_pr), ph1_R(sat_pr), snr_R(sat_pr), sat_pr, time, Eph_R, 1, iono); %#ok<ASGLU>
        else
            [pos_LS_SA, cov_pos_LS_SA, N_stim, cov_N_stim, PDOP, HDOP, VDOP] = code_phase_SA(pos_R(1:3), pr2_R(sat_pr), ph2_R(sat_pr), snr_R(sat_pr), sat_pr, time, Eph_R, 2, iono); %#ok<ASGLU>
        end
    else
        if (~isempty(Xhat_t_t))
            pos_LS_SA = Xhat_t_t([1,o1+1,o2+1]);
            pivot = 0;
        else
            pos_LS_SA = NaN(3,1);
        end
...
if isempty(cov_pos_LS_SA) %if it was not possible to compute the covariance matrix
    cov_pos_LS_SA = sigmaq0 * eye(3);
end
sigmaq_pos_LS_SA = diag(cov_pos_LS_SA);

%-------------------------------------------------------------------------------

Xhat_t_t = zeros(o3,1);
Xhat_t_t(1) = pos_LS_SA(1);
Xhat_t_t(o1+1) = pos_LS_SA(2);
Xhat_t_t(o2+1) = pos_LS_SA(3);
Cee(:,:) = zeros(o3);
Cee(1,1) = sigmaq_pos_LS_SA(1);
Cee(o1+1,o1+1) = sigmaq_pos_LS_SA(2);
Cee(o2+1,o2+1) = sigmaq_pos_LS_SA(3);