function [N_stim_slip, N_stim_born] = amb_estimate_LS(posR_app, posS, pr_R, pr_M, ...
    ph_R, ph_M, snr_R, snr_M, elR, elM, sat, sat_slip, sat_born, prRS_app, prMS_app, ...
...
% index vector of satellites with phase observations (over nsat)
p = [];
...
%PHASE
for i = 1 : nsat
    if (ph_R(i) ~= 0) & (ph_M(i) ~= 0) & (sat(i) ~= pivot)
...
Q(1:nsat-1,1:nsat-1) = sigmaq_cod1 * Q1;
if (nargin == 23)
