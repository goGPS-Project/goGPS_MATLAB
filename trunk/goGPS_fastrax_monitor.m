function goGPS_fastrax_monitor(filerootOUT)
...
%
% DESCRIPTION:
%   fastrax receiver monitor: stream reading, output data saving.
...
%set(rover,'InputBufferSize',115200);
...
while (rover_1 ~= rover_2) || (rover_1 == 0)
    ...
    %visualization
    fprintf('Fastrax: %7.4f sec (%4d bytes --> %4d bytes)\n', current_time, rover_1, rover_2);
...
f1 = figure;
s1 = get(0,'ScreenSize');
set(f1, 'position', [s1(3)-240-20 s1(4)-80-40 240 80], 'menubar', 'none', 'name', 'Fastrax monitor');
...
while flag
    ...
    if (rover_1 == rover_2) && (rover_1 ~= 0)
        ...
        for i = 1 : size(cell_rover,2)
            ...
            %TRACK message
            if (strcmp(cell_rover{1,i},'TRACK'))
            ...
            %PSEUDO message
            if (strcmp(cell_rover{1,i},'PSEUDO'))
                ...
                RangeEE_R  = cell_rover{3,i}(:,9);
                RateEE_R   = cell_rover{3,i}(:,10);
            ...
            if (strcmp(cell_rover{1,i},'FTX-EPH'))
        ...
        %visualization (PSEUDO information)
        if (nPSEUDO > 0)
